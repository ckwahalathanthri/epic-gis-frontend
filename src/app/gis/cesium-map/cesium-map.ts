...
export class CesiumMapComponent implements OnInit, OnDestroy {
      ...
      try {
        ...
        try {
          this.layerService.layerAdded$.subscribe((ref: any) => {
            ...
            if (looksLikeUrl) {
              this.addDataSourceFromUrl(urlOrId).catch(err => this.showError('Failed to load layer: ' + (err?.message || err)));
            } else {
              ...
              this.layerService.getGeoJson(urlOrId).subscribe({
                next: (geo: any) => {
                  try {
                    ...
                    this.addDataSourceFromUrl(tmpUrl).catch(err => this.showError('Failed to add fetched GeoJSON: ' + (err?.message || err)));
                  } catch (e) { this.showError('Invalid GeoJSON from backend'); }
                },
                error: (err: any) => this.showError('Failed to fetch converted GeoJSON from server: ' + (err?.message || err))
              });
            }
          });
        ...
        this.showError('Failed to create Cesium viewer: ' + (err?.message || err));
        console.error('Failed to create Cesium viewer', err);
      }
    }).catch(err => {
      this.showError('Failed to load Cesium script: ' + (err?.message || err));
  ...
  toggleTerrain() {
    ...
    if (this.terrainEnabled) {
      ...
      try {
        ...
        this.showError('Failed to enable world terrain; using ellipsoid.');
  ...
  loadTest() {
    ...
    this.addDataSourceFromUrl(this.testUrl).catch(err => this.showError('Test load failed: ' + (err?.message || err)));
  }

  private updateAttributionFromDataSource(ds: any) {
    try {
      const credits = ds.attribution || ds.source && ds.source.attribution;
  ...
  private updateAttributionFromTileset(tileset: any) {
    try {
      const meta = tileset.metadata || tileset.properties;
...
export class CesiumMapComponent implements OnInit, OnDestroy {
  ...
  @Component({
    ...
    template: `
      <div class="cesium-shell">
        ...
        <div class="cesium-controls">
          ...
          <button (click)="toggleTerrain()">{{ terrainEnabled ? 'Disable' : 'Enable' }} Terrain</button>
          <button (click)="toggleDebug()">{{ debug ? 'Hide' : 'Show' }} Debug</button>
          <div *ngIf="debug" style="margin-top:6px;display:flex;gap:6px;">
            <input [(ngModel)]="testUrl" placeholder="Paste public GeoJSON/KML/3DTiles URL" style="width:220px;padding:6px;border-radius:4px;border:1px solid rgba(255,255,255,0.06);background:transparent;color:#fff" />
            <button (click)="loadTest()">Load</button>
          </div>
        </div>
    ...
    styles: [`
      ...
      .cesium-controls {
        position:absolute; right:12px; top:12px; z-index:50; display:flex; flex-direction:column; gap:6px;
        background: rgba(6,24,40,0.7); padding:8px; border-radius:6px; color: #fff;
      }
      .cesium-controls button { background:transparent; border:1px solid rgba(255,255,255,0.08); color:#fff; padding:6px 8px; border-radius:4px; cursor:pointer; }
      .cesium-layer-list { position:absolute; left:12px; top:12px; z-index:50; background: rgba(6,24,40,0.7); padding:8px; border-radius:6px; color:#fff; max-width:260px; }
      .cesium-layer-list .title { font-weight:700; margin-bottom:6px; }
      .cesium-layer-list .last { margin-top:6px; font-size:0.85rem; opacity:0.9 }
    `]
  })
  export class CesiumMapComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
      this.loadCesium().then(() => {
        ...
        try {
          ...
          try {
            this.layerService.layerAdded$.subscribe((ref: any) => {
              // ref may be a URL string, an id, or an object provided by backend
              this.errorMessage = '';
              let urlOrId = ref;
              if (ref && typeof ref === 'object') {
                urlOrId = ref.url || ref.id || JSON.stringify(ref);
              }
              this.lastUrl = urlOrId;

              // If looks like a URL, attempt direct load; otherwise ask backend for GeoJSON
              const looksLikeUrl = typeof urlOrId === 'string' && /^(https?:)?\//.test(urlOrId);
              if (looksLikeUrl) {
                this.addDataSourceFromUrl(urlOrId).catch(err => this.showError('Failed to load layer: ' + (err && err.message ? err.message : err)));
              } else {
                // treat as ID or backend ref — call LayerService.getGeoJson
                this.layerService.getGeoJson(urlOrId).subscribe({
                  next: (geo: any) => {
                    // add GeoJSON blob/content via Blob URL
                    try {
                      const blob = new Blob([JSON.stringify(geo)], { type: 'application/geo+json' });
                      const tmpUrl = URL.createObjectURL(blob);
                      this.addDataSourceFromUrl(tmpUrl).catch(err => this.showError('Failed to add fetched GeoJSON: ' + err));
                    } catch (e) { this.showError('Invalid GeoJSON from backend'); }
                  },
                  error: (err: any) => this.showError('Failed to fetch converted GeoJSON from server: ' + (err?.message || err))
                });
    ...
    async addDataSourceFromUrl(url: string): Promise<void> {
      ...
      try {
        const ds = await Cesium.GeoJsonDataSource.load(url, { clampToGround: true });
        this.viewer.dataSources.add(ds);
        this.loaded.push({ type: 'geojson', ds, url });
        return;
      } catch (e) {
        // not geojson
      }
      ...
      try {
        const kds = await Cesium.KmlDataSource.load(url);
        this.viewer.dataSources.add(kds);
        this.loaded.push({ type: 'kml', ds: kds, url });
        return;
      } catch (e) {
        // not kml
      }
      ...
      try {
        const tileset = new Cesium.Cesium3DTileset({ url });
        this.viewer.scene.primitives.add(tileset);
        this.loaded.push({ type: '3dtiles', tileset, url });
        return;
      } catch (e) {
        // fallback
      }

      console.warn('Unsupported layer type for Cesium or failed to load:', url);
    }
    ...
    toggleTerrain() {
      ...
      if (this.terrainEnabled) {
        ...
        // try to enable Cesium World Terrain if available in assets
        try {
          this.viewer.terrainProvider = Cesium.createWorldTerrain();
          this.terrainEnabled = true;
        } catch (e) {
          console.warn('Failed to enable world terrain', e);
          try { this.viewer.terrainProvider = new Cesium.EllipsoidTerrainProvider(); } catch(_){}
          this.terrainEnabled = false;
        }
      }
