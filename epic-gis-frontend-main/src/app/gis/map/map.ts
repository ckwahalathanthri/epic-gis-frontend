...
(layer as any)._popupTemplate = popupTemplate;
  ...
  if (!this.view) return;
  try {
    // 1. Handle ✏️ Edit button click in the popup
    reactiveUtils.on(
      () => this.view.popup,
      'trigger-action',
      (event: any) => {
    ...
    // 2. Highlight the building/feature when the popup selects it
    reactiveUtils.watch(
      ...
      (feature: any) => {
        ...
        if (feature?.layer) {
          this.view.whenLayerView(feature.layer)
            .then((layerView: any) => {
    ...
    // 3. Remove highlight when popup closes — but keep it alive while the edit panel is open
    reactiveUtils.watch(
      () => this.view.popup?.visible,
      (visible: boolean) => {
        if (!visible && !this.showEditPanel && this.highlightHandle) {
          try { this.highlightHandle.remove(); } catch (_) {}
          this.highlightHandle = null;
        }
      }
  ...
  async toggleBuildingFootprints(enabled: boolean) {
    if (enabled) {
      if (!this.buildingLayer) {
        ...
        if (url) {
          // Popup template shown when any building is clicked — includes the ✏️ Edit action
          const buildingPopupTemplate: any = {
            title: '🏢 Building',
            content: [{ type: 'fields' }],   // shows all available fields automatically
            actions: [{ id: 'edit-feature', title: '✏️ Edit', className: 'esri-icon-edit' }]
          };

          if (url.includes('SceneServer')) {
             this.buildingLayer = new SceneLayer({
               url,
               outFields: ['*'],
